...
>对于某个样本空间已知而概率未知的概率空间 $(\Omega, \cal{F}, \cal{P})$，希望通过独立同分布的采样结果$\{X_i\} \sim \cal{P} (x)$ i.i.d 估计分布 $\cal{P}$。我们称采样结果${X_i}$为**数据**
...
> 对于给定的模型M，和给定的经验E，损失函数定义为函数$$\cal{L}:(M，E) \rightarrow \mathbb{R}$$ **损失函数值越大，代表人类认为模型M在任务T上的表现与经验E的表现差距越大**
